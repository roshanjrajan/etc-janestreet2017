...
def submit(t_t, symbol, orderType, price, number):
	orderID = random.randint(1, 10*10)
    ...
    while(1):
	    ...
	    symbol = message['symbol']
            ...
            if(v_tran = 2)
                ...
                if v_trade==1:
                    submit('add','VALBZ', 'BUY', VALBZ_price, min(VALBZ_number,VALE_number)
                    submit('add','VALE', 'SELL', VALE_price, min(VALBZ_number,VALE_number)
                elif v_trade==0:
                    submit('add','VALE', 'BUY', VALE_price, min(VALBZ_number,VALE_number)
                    submit('add','VALBZ', 'SELL', VALBZ_price, min(VALBZ_number,VALE_number)
                v_tran = 0
